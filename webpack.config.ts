...
function watch_it(compiler: webpack.Compiler) {
  if (compiler.options.watch) {
    if (!io) {
      ...
      console.info(`[Listener] 酒場監視サービスを起動しました, 監視中: http://0.0.0.0:${port}`);
      io.on('connect', socket => {
        console.info(`[Listener] 酒場ページ '${socket.id}' に接続しました, プッシュ初期化...`);
        io.emit('iframe_updated');
        socket.on('disconnect', reason => {
          console.info(`[Listener] 酒場ページ '${socket.id}' との接続が切断されました: ${reason}`);
        });
      });
    }

    compiler.hooks.done.tap('updater', () => {
      console.info('\n[Listener] コンパイル完了を検出, 更新イベントをプッシュ...');
...
const SchemaDump = {
  ...
  /** src ディレクトリに schema.ts が存在するか検出する */
...
function dump_schema(compiler: webpack.Compiler) {
  // シングルトンパターン + 必要に応じて実行：schema.ts が存在する場合のみ一度だけ初期化する
  ...
  const execute = () => {
    ...
    console.info('\x1b[36m[schema_dump]\x1b[0m すべての schema.ts を schema.json に変換しました');
  };
...
function resolveEntryProjectName(entry: Entry) {
  ...
  if (!projectName) {
    throw new Error(`なし法从入口路径解析子项目名称: ${entry.script}`);
  }
...
/**
 * resourceQuery 付きのルールセットを生成する
 * @description ?raw と ?url インポートの統一処理に使用する
 */
...
/**
 * HTML 入口なしの場合のスタイルルール
 * @description vue-style-loader と style-loader を使用する
 */
...
/**
 * HTML 入口ありの場合のスタイルルール
 * @description MiniCssExtractPlugin.loader を使用して CSS を抽出する
 */
