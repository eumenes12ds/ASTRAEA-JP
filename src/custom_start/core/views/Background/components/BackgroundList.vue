...
const checkRequirement = (required_value: string | undefined, current_value: string): boolean => {
  ...
  // 要求がある場合は完全一致必須（カスタムは満たしているとみなさない）
...
const getSelectButtonText = (item: Background) => {
  ...
  if (!meetsRequirements(item)) return '条件不一致';
  return '選択';
};
...
// カスタム開始シナリオかどうかを確認
const isCustomBackground = (item: Background) => item.name === '【カスタム開始】';
  ...
  <div class="background-list">
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <div class="custom-input-label">カスタム開始シナリオを書いてください：</div>
        <FormTextarea
          :model-value="customDescription"
          :rows="6"
          placeholder="ここで想像力を発揮し、自分だけのキャラクター初期シナリオを書いてください...&#10;&#10;例：キャラクターの出自、経験したイベント、現在の状況などを説明できます。"
