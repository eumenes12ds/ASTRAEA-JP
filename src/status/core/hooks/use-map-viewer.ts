...
export const useMapViewer = ({
  ...
  useEffect(() => {
    ...
    viewer.addHandler('open-failed', event => {
      const message = event?.message || '地図リソースのオープンに失敗';
  ...
  useEffect(() => {
    ...
    const waitForVisibleReady = (openedViewer: OpenSeadragon.Viewer) => {
      ...
      // 互換フォールバック: whenFullyLoaded が無い場合、最低でも次の描画フレームまで待つ
    ...
    const openFromCache = async () => {
      try {
        ...
        const normalizedError = error instanceof Error ? error : new Error('地図の読み込みに失敗');
        console.error('[MapViewer] 地図の読み込みに失敗:', normalizedError);
