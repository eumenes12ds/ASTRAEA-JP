...
export const getChatPartnerGalleryMap = (partner_names: string[]) => {
  ...
  try {
    ...
    console.log('[PartnerGallery] chat のパートナー画像の読み込みを開始:', partner_names);
    ...
    return partner_names.reduce<Record<string, PartnerGalleryItem[]>>((result, partner_name) => {
      ...
      if (galleryItems.length > 0) {
        console.log('[PartnerGallery] chat のパートナー画像にヒット:', partner_name, galleryItems);
    ...
    console.warn('[PartnerGallery] チャット変数のパートナー画像の読み込みに失敗:', error);
    return {};
  }
};
