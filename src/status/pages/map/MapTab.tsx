...
export const MapTab: FC = () => {
  ...
  useEffect(() => {
    ...
    // マーカー切り替え時にカードサイズのキャッシュをリセットし、markerCardReady が測定フローを再実行するようにする
  ...
  useEffect(() => {
    try {
      ...
      console.error('[StatusMap] 地図の描画/マーカーの読み込みに失敗:', error);
    }
  }, [setMapMarkers]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        console.error('[StatusMap] 地図の描画保存に失敗:', error);
      }
    }, 1000);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        console.error('[StatusMap] 地図マーカーの保存に失敗:', error);
      }
    }, 1000);
  ...
  useLayoutEffect(() => {
    ...
    const tryObserve = () => {
      ...
      if (!el) {
        // Portal の ref がまだマウントされていない。次のフレームで再試行
  ...
  useMapViewer({
    ...
    onError: error => {
      ...
      setMapLoadError(error.message || '地図の読み込みに失敗');
    },
  });
