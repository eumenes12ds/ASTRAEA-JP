...
declare namespace SillyTavern {
  type ChatMessage = {
    ...
    is_user: boolean;
    /**
     * 実際には、メッセージが非表示で llm に送信されないことを示す
     */
    is_system: boolean;
...
/**
 * 酒場がプラグインに提供する安定したインターフェース, 詳細は SillyTavern/public/scripts/st-context.js または https://github.com/SillyTavern/SillyTavern/blob/release/public/scripts/st-context.js を参照
 * 酒場のページで f12 を押し、コンソールに `window.SillyTavern.getContext()` と入力すると、現在の酒場が提供するインターフェースを確認できる
 */
declare const SillyTavern: {
  ...
  /** `/inject`、`setExtensionPrompt` などで注入されたすべての追加プロンプト */
   ...
   * 追加のプロンプトを1つ注入する
   *
   * @param prompt_id id, 重複すると元の内容を置き換える
   * @param content 内容
   * @param position 位置. -1 は注入しない (scan=true と組み合わせてグリーンライトの発動にのみ使用), 1 はチャットに挿入する
   * @param depth 深度
   * @param scan スキャン対象テキストとして、ワールドブックの緑ランプエントリのスキャン対象テキストに含めるかどうか
   * @param role メッセージのロール. 0 は system, 1 は user, 2 は assistant
   * @param filter プロンプトがどのような場合に有効になるか
   */
  ...
  readonly registerFunctionTool: (tool: {
    ...
    /** ツールの説明 */
    description: string;
    /** 関数の引数に対する JSON schema 定義, zod の z.toJSONSchema で取得できる */
    parameters: Record<string, any>;
    /** 登録する関数呼び出しツール */
    action: ((args: any) => string) | ((args: any) => Promise<string>);

    /** 関数呼び出しの結果メッセージをどのようにフォーマットするか; デフォルトでは何も行わず、`'Invoking tool: ツールの表示名'` と表示される */
    formatMessage?: (args: any) => string;
    /** 次のチャット補完リクエストでこのツールを登録するかどうか; デフォルトは常に登録 */
    shouldRegister?: (() => boolean) | (() => Promise<boolean>);
    /** フロア内で関数呼び出しの結果を1フロア使って表示しないかどうか, `true` の場合は表示されず生成も発火しない; デフォルトは false */
    stealth?: boolean;
  }) => void;
  ...
  /** このメソッドでキャラクターカードの `extensions` フィールドを変更することは固く禁じられている, 元の値と新しい値を置換ではなくマージするため; `updateCharacterWith` を使用すべきだ */
